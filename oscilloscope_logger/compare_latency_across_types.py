...
def compare_latency_across_types(files_dict, output_dir="oscilloscope_latency_comparison"):
    ...
    for label, (file, color) in files_dict.items():
        ...
        plt.plot(range(1, len(latency)+1), latency, label=label, color=color, marker='o', markersize=7, linewidth=2, alpha=0.85)
    plt.xlabel("Packet Attempt", fontsize=29)
    plt.ylabel("Latency (ms)", fontsize=29)
    plt.title("Packet Response Times for 100 Tests with Oscilloscope at 60V with 120° Conduction Angle", fontsize=29)
